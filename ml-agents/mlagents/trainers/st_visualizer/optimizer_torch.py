...
class STVisualizationActor(SuperTrackPolicyNetwork):
    ...
    def get_action_and_stats(
        ...
        action, log_probs, entropies, _means = self.action_model(encoding, None) 
        run_out = {}
        ...
        continuous_clipped_action = action.to_action_tuple(
            clip=self.action_model.clip_action,  output_scale=self.output_scale
        ).continuous
        cont_clipped_action_tensor = torch.from_numpy(continuous_clipped_action).to(default_device())
        final_tensor = torch.cat((cont_clipped_action_tensor, torch.unsqueeze(final_flat_tensor,0)), dim=-1)

        run_out["env_action"] = ActionTuple(continuous=final_tensor.cpu().numpy())
    ...
    def parse_world_model_data(self, world_model_data): # shape [2672]
        nframes = self.nframes
        # B = 1
        # root_poses = torch.empty(nframes, 3)
        # root_rots = torch.empty(nframes, 4)
        k_pos = torch.empty(nframes, NUM_T_BONES, 3)
        k_rot = torch.empty(nframes, NUM_T_BONES, 4)
        k_vel = torch.empty(nframes, NUM_T_BONES, 3)
        k_rvel = torch.empty(nframes, NUM_T_BONES, 3)
        ...
        for frame in range(nframes):
            for bone in range(NUM_T_BONES):
                k_pos[frame, bone, :] = world_model_data[idx:idx+3]
                idx += 3
                k_rot[frame, bone, :] = world_model_data[idx:idx+4]
                idx += 4
                k_vel[frame, bone, :] = world_model_data[idx:idx+3]
                idx += 3
                k_rvel[frame, bone, :] = world_model_data[idx:idx+3]
                idx += 3
                pd_rots[frame, bone, :] = world_model_data[idx:idx+4]
                idx += 4
                pd_rvels[frame, bone, :] = world_model_data[idx:idx+3]
                idx += 3
    ...
    def get_predictions_from_wm(self, st_data, world_model_data, policy_action):
        nframes = self.nframes
        predicted_bone_poses = torch.empty(nframes, NUM_T_BONES, 3) 
        predicted_bone_rots = torch.empty(nframes, NUM_T_BONES, 4) 
        B = 1
        ...
        sim_state = [torch.stack(t)[:, 1:, :]  for t in zip(*sim_inputs)]
        ...
        for i in range(nframes):
            ...
            # Use .unsqueeze(0) to add batch dim
            kin_state = k_pos[i].unsqueeze(0), k_rot[i].unsqueeze(0), k_vel[i].unsqueeze(0), k_rvel[i].unsqueeze(0)
    ...
    def get_policy_action(self, sim_state, kin_state):
        ...
        input = [*local_kin, *local_sim]
        B = 1
        if self.policy_includes_global_data:
                global_drift = torch.empty(B, 3)
                sim_hip_world_pos = sim_state[0][0, 0, :]
                kin_hip_world_pos = kin_state[0][0, 0, :]
                global_drift = (kin_hip_world_pos - sim_hip_world_pos).unsqueeze(0)
        action, determinstic_action = self.get_action_during_training(input, global_drift=global_drift)
        return action